...
export class LoginComponent {
    ...
    login() {
        ...
        if (this.isJSON(this.fileContent)) {
            ...
            const toURLSubject = this.socketManageService.getMessageSubject("user", "to_url").subscribe(
                message => {
            ...
            this.socketManageService.send("user", "login", {
    ...
    async keyExchange() {
        ...
        if (this.isJSON(this.fileContent)) {
            ...
            this.socketManageService.initializeMainConnection(serverAddress)
                .then(async () => {
                    try {
                        ...
                        // const keyExchangeSubject = this.socketManageService.getMessageSubject("key", "key_exchange").subscribe(
                        //     message => {
                        //         const data = JSON.parse(message.data).messages;
                        ...
                        //     }
                        // )
                        // this.socketManageService.send("key", "key_exchange", {
                        //     "public_key": publicKeyString
                        // })
                        self.login()
                    } catch (err) {
                        this.socketManageService.disconnect()
                        console.error(err);
                    }
                })
                .catch((error) => {
